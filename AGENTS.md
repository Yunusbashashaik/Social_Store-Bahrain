...
**GoDaddy:** Admin requires the Node process (`npm run build && npm start`). Empty disks restore from packaged `catalog-backup/` and the public raw URL `https://raw.githubusercontent.com/Yunusbashashaik/Social_Store-Bahrain/main/catalog-backup/admin-state.json` without a token. Optional: `CATALOG_BACKUP_TOKEN` + `CATALOG_BACKUP_REPO` to **push** admin edits. Never set `ALLOW_FACTORY_SEED` in production. Static FTP uploads cannot serve `/api/admin/login` and will show “Load failed”. Verify `GET /api/health` on the live domain. If the API is on another host, set `apiUrl` in `client/public/runtime-config.js`.
